...
class ClassTools
{
    ...
    {
        if (false !== $pos = strrpos(strval($qualifiedName), '.')) {
            return substr(strval($qualifiedName), $pos + 1); // start just after '.'
        } elseif (false !== $pos = strrpos(strval($qualifiedName), '\\')) {
            return substr(strval($qualifiedName), $pos + 1);
        } else {
